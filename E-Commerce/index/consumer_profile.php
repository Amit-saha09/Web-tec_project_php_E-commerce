...
<head>
    ...
    <title>Consumers Profile page</title>
...
<?php
    ...
    else
    {
        ...
        $stmt1 = $conn1->prepare("select * from consumer where email=?");
    ...
    if ($_SERVER["REQUEST_METHOD"] =="POST" ) 
    {
        ...
        if ($count >= 5)
        {
            ...
            $stmt1 = $conn1->prepare("update consumer set fname=?, lname=?, password=?, number=?, address=? where email=?");
            $stmt1->bind_param("ssssss", $firstName, $lastName, $password, $phone, $address, $email);
...
<script>
			...
			function validate() {
                ...
                {
					document.getElementById("errorMsg").innerHTML = "<b>Please fill up the Update form properly.</b>";
